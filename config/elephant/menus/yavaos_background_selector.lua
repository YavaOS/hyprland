Name = "yavaosBackgroundSelector"
NamePretty = "YavaOS Background Selector"
Cache = false
...
function GetEntries()
  ...
  local theme_name_file = io.open(home .. "/.config/yavaos/current/theme.name", "r")
  ...
  local dirs = {
    home .. "/.config/yavaos/current/theme/backgrounds",
  }
  if theme_name then
    table.insert(dirs, home .. "/.config/yavaos/backgrounds/" .. theme_name)
  end
      ...
      for background in handle:lines() do
        ...
        if filename and not seen[filename] then
          ...
          table.insert(entries, {
            ...
            Value = background,
            Actions = {
              activate = "yavaos-theme-bg-set " .. ShellEscape(background),
            },
            Preview = background,
            PreviewType = "file",
          })
